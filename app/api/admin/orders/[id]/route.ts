...
export async function GET(request: Request, { params }: Context) { if (!(await requireAdmin(request))) return NextResponse.json({ message: "Unauthorized." }, { status: 401 }); const order = await prisma.order.findUnique({ where: { id: (await params).id }, include: { items: { include: { product: true } }, customer: true, payment: true, shippingAddress: true } }); return order ? NextResponse.json(order) : NextResponse.json({ message: "Order not found." }, { status: 404 }); }
