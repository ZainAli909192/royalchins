...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
 ...
 const address = await prisma.$transaction(async (tx: Prisma.TransactionClient) => { if (parsed.data.isDefault) await tx.customerAddress.updateMany({ where: { customerId: session.sub }, data: { isDefault: false } }); return tx.customerAddress.update({ where: { id }, data: parsed.data }); }); return NextResponse.json(address);
}
