...
export async function POST(request: Request) {
 ...
 const reviewSetting = await prisma.storeSetting.findUnique({ where: { key: "reviews" }, select: { value: true } }); const autoApprove = typeof reviewSetting?.value === "object" && reviewSetting.value !== null && !Array.isArray(reviewSetting.value) && reviewSetting.value.autoApproveReviews === true;
 try { return NextResponse.json(await prisma.review.create({ data: { customerId: session.sub, productId: input.productId, orderId: order.id, rating: input.rating, title: input.title, comment: input.comment, status: autoApprove ? "Approved" : "Pending", ...(autoApprove ? { moderatedAt: new Date(), moderatedBy: "Automatic moderation" } : {}) } }), { status: 201 }); } catch { return NextResponse.json({ message: "You have already reviewed this product for this order." }, { status: 409 }); }
}
