...
type PaymentData = {
  id: string; provider: string; providerPaymentIntentId: string | null; amount: string | number; currency: string;
...
export default function PaymentDetailsPage() {
  ...
  return <div className="space-y-6">
    <AdminPageHeader title={`Payment PAY-${payment.id.slice(-6).toUpperCase()}`} description={`Stripe transaction ${payment.providerPaymentIntentId ?? "pending"}`} action={<Button variant="outline" onClick={() => router.push("/admin/payments")}><ArrowLeft className="mr-2 h-4 w-4" />Back</Button>} />
    ...
    <div className="grid gap-6 lg:grid-cols-2"><section className="rounded-xl border border-border bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">Transaction details</h2><div className="mt-4 divide-y divide-border"><Row label="Provider" value={payment.provider} /><Row label="Payment intent" value={payment.providerPaymentIntentId ?? "Not created"} /><Row label="Status" value={payment.status} /><Row label="Failure reason" value={payment.failureMessage ?? "—"} /></div>{payment.receiptUrl && <a className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-primary hover:underline" href={payment.receiptUrl} target="_blank" rel="noreferrer">Open Stripe receipt <ExternalLink className="h-4 w-4" /></a>}</section>
