    ...
    <div className="mx-auto max-w-[1440px] px-4 py-5 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
      <div className="mb-6 sm:mb-8">
        ...
        <div className="mt-1 flex flex-wrap items-end justify-between gap-3">
          ...
          <Link
            href="/"
            className="hidden text-sm font-semibold text-primary transition-colors hover:text-primary-hover sm:inline-flex"
          >
            Continue Shopping
          </Link>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_400px] lg:gap-8">
        <section className="space-y-3 sm:space-y-4">
          <div className="hidden items-center justify-between border-b border-border pb-3 sm:flex">
            <p className="text-sm font-bold text-foreground">Cart items</p>
            <p className="text-xs font-medium text-muted-foreground">Review your selection before checkout</p>
          </div>
          {items.map((item) => (
            <CartItemCard
              key={item.id}
        ...
        <aside className="h-fit rounded-3xl border border-border bg-background p-5 shadow-md lg:sticky lg:top-24 lg:p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Checkout</p>
              <h2 className="mt-1 text-xl font-bold text-foreground">Order summary</h2>
            </div>
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 text-primary">
              <ShoppingBag aria-hidden="true" className="h-5 w-5" strokeWidth={1.9} />
            </span>
          </div>
          ...
          <Button
            ...
            className="mt-5 flex h-13 w-full items-center justify-center rounded-xl text-sm font-bold shadow-primary transition-[transform,box-shadow] duration-200 hover:-translate-y-0.5 hover:shadow-md active:translate-y-0 motion-reduce:transform-none"
          >
          ...
          <div className="mt-5 flex items-start gap-3 rounded-2xl border border-border bg-surface-subtle p-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-muted-foreground">
                Delivery details and fees are confirmed during checkout.
              </p>
            </div>
          </div>
          ...
          <p className="mt-4 flex items-center justify-center gap-1.5 text-center text-[11px] leading-5 text-muted-foreground">
            ...
            Secure checkout. Animal quantities are limited to one per cart line.
          </p>
        </aside>
      </div>
      ...
      <div className="mt-10 border-t border-border pt-8 sm:mt-12 sm:pt-10">
        <FeaturedProducts
          products={initialItems.map(({ slug, name, image, type, price, shortMeta }) => ({
            slug,
            name,
            image,
            type,
    ...
    <div className="mx-auto flex min-h-[65vh] max-w-[600px] items-center justify-center px-4 py-12">
      <div className="w-full rounded-3xl border border-border bg-background p-8 text-center shadow-sm sm:p-12">
        ...
        <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-muted-foreground">
          Explore available companions and accessories and add something to your
          cart.
        </p>

        <Link
          href="/"
          className="mt-6 inline-flex h-12 items-center justify-center rounded-xl bg-primary px-6 text-sm font-semibold text-primary-foreground shadow-primary transition-[transform,background-color] duration-200 hover:-translate-y-0.5 hover:bg-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 motion-reduce:transform-none"
        >
          Browse Products
        </Link>
      </div>
