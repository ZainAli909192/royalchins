...
export function StripeCardForm({ total, onPaymentSucceeded }: StripeCardFormProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Button type="button" variant="primary" disabled={!stripe || !elements || isProcessing} onClick={handlePayment} className="h-12 w-full rounded-xl px-6 text-sm font-bold disabled:cursor-not-allowed disabled:opacity-50">
      ...
      <div className="flex items-start justify-center gap-2"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><p className="max-w-md text-center text-[11px] leading-5 text-muted-foreground">Stripe securely collects the card number, name, expiry date, and CVV. Royal Chins never receives or stores complete card details.</p></div>
    </div>
  );
}
